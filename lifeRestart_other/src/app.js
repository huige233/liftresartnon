...
class App {
    ...
    async initial() {
        this.initPages();
        this.switch('loading');
        const [, specialthanks] = await Promise.all([this.#life.initial(), json('specialthanks')]);
    ...
    initPages() {
        ...
        propertyPage.mounted = () => {
            propertyPage.find('#talentSelectedView').append(
                `<li>已选天赋</li>${Array.from(this.#talentSelected)
                    .map(
                        ({ name, description }) =>
        ...
        trajectoryPage
            ...
            .click(() => {
                ...
                domtoimage.toJpeg(document.getElementById('lifeTrajectory')).then(dataUrl => {
                ...
                clear: () => {
                    ...
                    Array.from(this.#talentSelected)
                        .sort(({ id: a, grade: ag }, { id: b, grade: bg }) => {
                            if (a == lastExtendTalent) {
                                return -1;
                            }
                            if (b == lastExtendTalent) {
                                return 1;
                            }
                            return bg - ag;
                        })
