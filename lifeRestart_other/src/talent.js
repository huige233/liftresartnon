...
class Talent {
    ...
    get(talentId) {
        ...
        if (!talent) {
            throw new Error(`[ERROR] No Talent[${talentId}]`);
        }
        return clone(talent);
    }
    ...
    talentRandom(include, { times = 0, achievement = 0 } = {}) {
        ...
        const randomGrade = () => {
            ...
            if ((randomNumber -= rate[3]) < 0) {
                return 3;
            }
            if ((randomNumber -= rate[2]) < 0) {
                return 2;
            }
            if ((randomNumber -= rate[1]) < 0) {
                return 1;
            }
            return 0;
        };
        ...
        const getReplaceList = (talent, talents) => {
            ...
            if (replacement.grade) {
                this.forEach(({ id, grade }) => {
                    if (!replacement.grade[grade]) {
                        return;
                    }
                    if (this.exclusive(talents, id)) {
                        return;
                    }
