...
async function write(sheets) {
    for (const sheetName in sheets) {
        ...
        console.info(
            '[Transform] XLSX(',
            source.map(([p, s]) => `${p}:${s}`).join('\n\t\t  '),
            `) \n\t -> JSON( ${savePath} )`
        );
        await writeFile(savePath, JSON.stringify(data, null, 4));
    }
}
...
function format(rawSheet, isArray, xlsxPath, rawSheetName) {
    ...
    for (const index in rawSheet) {
        ...
        if (mainKey === undefined) {
            console.warn(
                `[WARN][No Main Key] ${xlsxPath}:${rawSheetName}`,
                parseInt(index) + 3,
                rowData
            );
            continue;
        ...
        if (newSheet[mainKey]) {
            console.warn(
                `[WARN][Duplicate Key] ${mainKey} ${xlsxPath}:${rawSheetName}`,
                parseInt(index),
                '\n\t',
...
function merge(original, rawData, isMerge, isArray, xlsxPath, rawSheetName) {
    ...
    if (!isMerge) {
        if (original) {
            console.warn(
                `[WARN][Sheet Duplicate] ${xlsxPath}:${rawSheetName}\n\t\t${original.source[0][0]}:${original.source[0][1]}`
            );
            return original;
        }
    }
    ...
    if (original.isArray != isArray) {
        console.warn(
            `[WARN][Sheet Format not pair] ${xlsxPath}:${rawSheetName}\n\t\t${original.source[0][0]}:${original.source[0][1]}`
        );
        return original;
    }
    ...
    if (isArray) {
        ...
        for (const key in formatData) {
            if (original.data[key]) {
                console.warn(
                    `[WARN][Duplicate key] ${key} ${xlsxPath}:${rawSheetName} ${JSON.stringify(formatData[key])}\n\t\t${original.source[0][0]}:${original.source[0][1]} ${JSON.stringify(original.data[key])}`
                );
                continue;
            }
